...
internal sealed class GetSubmissionBatchById : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet("/submissions/batches/{submissionBatchId:int}", async (
            int submissionBatchId,
            ISubmissionBatchService service) =>
        {
            var result = await service.GetSubmissionBatchByIdAsync(submissionBatchId);
            return result.Match(
                Results.Ok,
